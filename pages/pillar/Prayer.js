...
export function render(outlet) {
    ...
    initPrayerTool();
    // cleanup: clear the countdown interval when navigating away
...
<div class="content-narrow">
  ...
  <div class="card" style="display:flex;gap:2rem;align-items:center;flex-wrap:wrap">
    <!-- COMPASS — fixed geometry -->
    <div class="qibla-wrap">
      <!--
        Compass geometry:
        - .compass-ring is 240×240px, position:relative
        - .compass-pivot sits at the exact centre (top:50%, left:50%)
          with width/height 0 so it doesn't occupy space
        - .compass-needle-arm is the rotating child:
            width: 4px, height: 100px
            positioned so its BOTTOM is at the pivot (margin-left:-2px, margin-top:-100px)
            transform-origin: bottom center → rotates around the pivot point
        - The Kaaba emoji is absolutely centred on the ring independently
      -->
      <div class="compass-ring" id="compass">
        ...
        <!-- Pivot wrapper: zero-size element at exact compass centre -->
        <div id="qibla-pivot" style="
          position:absolute;
          top:50%; left:50%;
          width:0; height:0;
        ">
          <!-- Needle arm: extends 100px upward from the pivot -->
          <div id="qibla-needle-arm" style="
            position:absolute;
            bottom:0; left:0;
            width:4px; height:100px;
            margin-left:-2px;
            background:linear-gradient(to top, rgba(27,92,66,0.15) 0%, var(--green) 100%);
            border-radius:2px 2px 0 0;
            transform-origin:bottom center;
            transform:rotate(0deg);
            transition:transform 1s cubic-bezier(.4,0,.2,1);
          "></div>
          <!-- Tail: extends 40px downward (opposite direction) -->
          <div aria-hidden="true" style="
            position:absolute;
            top:0; left:0;
            width:4px; height:40px;
            margin-left:-2px;
            background:linear-gradient(to bottom, rgba(180,180,180,0.6) 0%, transparent 100%);
        ...
        <!-- Kaaba icon: always stays centred, above the needle -->
        <div class="compass-kaaba" aria-label="Ka'bah direction indicator">🕋</div>
    ...
    <div style="flex:1;min-width:200px">
      ...
      <div style="background:var(--green-light);border:1px solid #B0D8C4;border-radius:var(--radius-sm);padding:10px 12px;font-size:12.5px;color:var(--green);line-height:1.5">
        💡 Face the bearing shown above when standing upright. The needle points toward Makkah — align your body so you face the green end.
      </div>
    </div>
  </div>
...
function initPrayerTool() {
    ...
    // Pre-fill from shared state if location is already known
    if (state.location) {
    ...
    function calculate() {
        ...
        if (isNaN(lat) || isNaN(lng)) {
            statusEl.textContent = 'Please enter valid coordinates (decimal degrees).';
            return;
        }
        ...
        function updateCountdown() {
            // Re-derive next prayer on every tick so the label + minutes stay live
        ...
        document.getElementById('qibla-dir').textContent =
            `${bearingToLabel(bearing)} from your location`;
        document.getElementById('qibla-dist').textContent =
            `${dist.toLocaleString()} km`;

        // ── FIXED NEEDLE ROTATION ──────────────────────────────────────
        // The pivot div sits at exactly the compass centre (top:50%, left:50%).
        // The needle arm extends 100 px upward from that pivot (bottom:0).
        // transform-origin:bottom center makes it rotate around the pivot.
        // We ONLY set the rotation — no translateX needed on the arm itself.
